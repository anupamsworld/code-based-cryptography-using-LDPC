...
def hex_to_binary(hex_str):
    ...
    #print(f"bin(int(hex_str, 16))={bin(int(hex_str, 16))}")
    return binary_str
...
#print(f"Hexadecimal {hex_value} is binary {binary_value}")

binary_value_list = list(map(int, list(binary_value)))
#print(f"type(binary_value)={type(binary_value)}")

#print(f"type(binary_value_list)={type(binary_value_list)}")
#print(f"binary_value_list={binary_value_list}")

G_list = [[0]*8176 for _ in range(7154)]

for i in range(7154):
    G_list[i][i] = 1


df=circulants_linear[0]
#print(f"df={type(df)}")
#print(f"{df}")


rows, cols = 14, 2
circulants = [[circulants_linear[i*cols + j] for j in range(cols)] for i in range(rows)]
#print(circulants)

del circulants_linear

#print(f"list(map(int, list(hex_to_binary(circulants[i][j]))))={list(map(int, list(hex_to_binary(circulants[0][0]))))}")

#exit()

# As we have filled the curcukant matrix with the
# column elements from original given matrix, in row wise manner.
...
for i in range(14):
    for j in range(2):
        '''
        G_list_row = i*511
        G_list_column = 8176-1022-1
        #G_list[G_list_row : G_list_row+1] [G_list_column : G_list_column+511] = list(map(int, list(hex_to_binary(circulants[i][0]))))
        binary=list(map(int, list(hex_to_binary(circulants[i][0]))))
        for x in range(G_list_row, G_list_row + 1):
            for index, y in zip(range(511), range(G_list_column, G_list_column+511)):
                G_list[x][y] = binary[index]
        

        G_list_row = i*511
        G_list_column = 8176-1022-1 + 511
        #G_list[G_list_row : G_list_row+1] [G_list_column : G_list_column+511] = list(map(int, list(hex_to_binary(circulants[i][1]))))
        binary=list(map(int, list(hex_to_binary(circulants[i][1]))))
        for x in range(G_list_row, G_list_row + 1):
            for index, y in zip(range(511), range(G_list_column, G_list_column+511)):
                G_list[x][y] = binary[index]
        '''
        G_list_row = i*511
        G_list_column = 8176-1022 + (j*511)
        #G_list[G_list_row : G_list_row+1] [G_list_column : G_list_column+511] = list(map(int, list(hex_to_binary(circulants[i][1]))))
        binary=list(map(int, list(hex_to_binary(circulants[i][j]))))
        binary.pop(0)
        for x in range(G_list_row, G_list_row + 1):
            for index, y in zip(range(511), range(G_list_column, G_list_column+511)):
                G_list[x][y] = binary[index]
    ...
    for j in range(2):
        ...
        for k in range(510): # row index is (i*511)+k
            for l in range(511): # column index is (j*511)+l
                G_list[G_list_row+k+1][G_list_column+((l+1)%511)] = G_list[G_list_row+k][G_list_column+l]
...
with open(PWD+'/circulants_linear_for_G.txt', 'w') as file:
    for i in range(7154):
        for j in range(8176):
            file.write(str(G_list[i][j]))
        file.write("\n")
'''